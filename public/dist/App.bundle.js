...
function autocomplete(input, latInput, lngInput) {
  if (!input) return; // skip fn from runninng if there is no input
  ...
  _axios2.default.get('/api/stores/near?lat=' + lat + '&lng=' + lng).then(function (res) {
    ...
    var markers = places.map(function (place) {
      var _place$location$coord = _slicedToArray(place.location.coordinates, 2),
          ...
          placeLat = _place$location$coord[1]; // destructure array
    ...
    // when someone clicks on the markers
...
function typeAhead(search) {
  ...
  searchInput.on('keyup', function (e) {
    // if they aren't pressing
    if (![38, 40, 13].includes(e.keyCode)) {
      return; // skipp the other keycodes
    }
    ...
    if (e.keyCode === 40 && current) {
      next = current.nextElementSibling || items[0]; // fallback to first item
